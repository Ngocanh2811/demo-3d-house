...
// --- COMPONENT: CHỌN KHÁCH HÀNG (Dành cho Broker) ---
function BuyerSelector({ players, onClose, onSelect }) {
    ...
    return (
        <div style={{
            ...
            <div style={{background: "white", padding: "20px", borderRadius: "10px", width: "320px", boxShadow: "0 10px 30px rgba(0,0,0,0.3)"}}>
                <h3 style={{marginTop: 0, color: "#2c3e50"}}>📋 CHỌN KHÁCH CHỐT ĐƠN</h3>
                <p style={{fontSize:"12px", color:"#7f8c8d"}}>Chọn khách hàng đang online để ký hợp đồng.</p>
                <div style={{maxHeight: "200px", overflowY: "auto", border: "1px solid #eee", marginBottom: "15px", borderRadius:"4px"}}>
                    ...
                    clientList.map(p => (
                        <div key={p.id} onClick={() => onSelect(p.id)} style={{
                            ...
                            <div style={{width:"24px", height:"24px", borderRadius:"50%", background:p.color}}></div>
                            <b>{p.name}</b>
                        </div>
                    ))}
                </div>
...
// --- [ĐÃ KHÔI PHỤC] THÔNG TIN NHÀ CHI TIẾT ---
      ...
      {showInfo && (
        <Html position={[2, 4, 2]} center zIndexRange={[100, 0]}>
          <div style={{ background: "white", padding: "20px", borderRadius: "12px", width: "350px", textAlign: "left", boxShadow: "0 20px 50px rgba(0,0,0,0.5)", fontFamily: "Segoe UI", pointerEvents:"auto", overflow:"hidden" }}
            ...
            {/* [ĐÃ THÊM LẠI] GRID THÔNG TIN CHI TIẾT */}
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", fontSize:"13px", color:"#444", marginBottom:"15px"}}>
                <div style={{background:"#f8f9fa", padding:"5px", borderRadius:"4px"}}>💰 <b>Giá bán:</b><br/><span style={{color:"#c0392b", fontWeight:"bold"}}>$500,000</span></div>
                ...
                <div style={{background:"#f8f9fa", padding:"5px", borderRadius:"4px"}}>🛌 <b>Kết cấu:</b><br/>4 PN, 5 WC</div>
...
export default function App() {
  ...
  const handleConfirmSale = (buyerId) => {
      ...
      socket.emit("confirmTransaction", { buyerId }); 
  };
        ...
        <div style={{ flex: 1, overflowY: "auto", fontSize: "13px" }}>
          {chatList.map((msg, i) => (
             <div key={i} style={{ marginBottom: "8px", textAlign: msg.id === socket.id ? "right" : "left" }}>
                ...
                <div style={{ background: msg.isPrivate ? "#fff3cd" : (msg.id === socket.id ? "#3498db" : (msg.role==='bot'?"#9b59b6":"#d3b87dff")), color: msg.isPrivate ? "black" : (msg.id === socket.id ? "white" : "black"), padding: "8px 12px", borderRadius: "12px", display: "inline-block", maxWidth: "85%", border: msg.isPrivate ? "1px solid #e67e22" : "none", textAlign: "left" }}>
