...
io.on("connection", (socket) => {
  ...
  socket.emit("updateHouseStatus", houseStatus); // Chỉ gửi chuỗi status, không gửi data sổ hồng
  ...
  // --- [LOGIC MỚI] Xử lý chốt đơn bảo mật ---
  socket.on("confirmTransaction", ({ buyerId }) => {
    // 1. Cập nhật trạng thái nhà thành SOLD cho TOÀN BỘ server (để đổi màu biển báo)
    houseStatus = "SOLD";
    io.emit("updateHouseStatus", "SOLD");
    
    // 2. Tạo dữ liệu hợp đồng/sổ hồng
    ...
    io.emit("receiveMessage", { 
        ...
        text: `🔗 HỢP ĐỒNG ĐÃ ĐƯỢC KÝ KẾT VỚI ${txData.buyer}!` 
    });

    // 4. Gửi Sổ hồng (Private) - CHỈ Broker và Khách mua mới nhận được
    io.to(socket.id).emit("showCertificate", txData); // Gửi cho Broker
    if (buyerId && players[buyerId]) {
        io.to(buyerId).emit("showCertificate", txData); // Gửi cho Khách mua
    }
  });

  // Reset trạng thái nhà (Cho Broker test lại)
