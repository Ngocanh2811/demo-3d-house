...
io.on("connection", (socket) => {
  ...
  socket.on("sendMessage", (data) => {
    ...
    const messageData = { id: socket.id, name: sender?.name, role: sender?.role, text: data.text, isPrivate: data.isPrivate, recipient: data.recipient };

    if (data.isPrivate) {
        // 1. Gửi lại cho người gửi (để họ thấy tin mình vừa nhắn)
        socket.emit("receiveMessage", messageData);

        // 2. Tìm ông Sales (Broker) để gửi (cả sales có thể nhắn riêng cho khách)
        const recipientId = data.isPrivate && sender?.role === "broker" ? 
                            Object.keys(players).find(key => players[key].role === "client" && players[key].name === data.recipient) :
                            Object.keys(players).find(key => players[key].role === "broker");
        
        if (recipientId && recipientId !== socket.id) {
