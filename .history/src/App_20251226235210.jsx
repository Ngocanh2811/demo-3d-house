...
// ⚠️ KHÔNG CẦN IMPORT QRCode NỮA ĐỂ TRÁNH LỖI CRASH
// import QRCode from "react-qr-code"; 

const socket = io.connect("http://localhost:3001");

// --- COMPONENT: SỔ HỒNG SỐ (PHIÊN BẢN AN TOÀN - KHÔNG CRASH) ---
function DigitalCertificate({ data, onClose }) {
  // 1. Kiểm tra an toàn: Nếu không có data thì không render gì cả (tránh màn hình trắng)
  if (!data) return null;

  // 2. Tạo link QR bằng API online (Không cần cài thư viện)
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=BLOCKCHAIN-TX:${data.hash}`;
      ...
      <div style={{
        ...
        <div style={{display: "flex", justifyContent: "space-between", alignItems: "flex-start"}}>
            ...
            <div style={{flex: 1, paddingRight: "20px"}}>
                <p style={{fontSize: "18px", margin:"0"}}>Chứng nhận tài sản NFT thuộc về:</p>
                {/* Dùng toán tử ?. để tránh lỗi nếu data chưa load kịp */}
                <h1 style={{color: "#2c3e50", margin: "10px 0", fontStyle: "italic"}}>{data?.buyer || "Đang cập nhật..."}</h1>
                ...
                <div style={{marginTop: "20px", fontSize: "16px", lineHeight: "1.6"}}>
                    ...
                    {/* Kiểm tra data.price có tồn tại không trước khi format */}
                    <div>💰 <b>Giá trị:</b> <span style={{color: "#c0392b", fontWeight: "bold"}}>${data?.price ? data.price.toLocaleString() : "0"}</span></div>
                    <div>🕒 <b>Thời gian:</b> {data?.timestamp || "Vừa xong"}</div>
                </div>
                ...
                <div style={{marginTop: "20px", background: "#f8f9fa", padding: "10px", borderRadius: "4px", border: "1px dashed #ccc"}}>
                    ...
                    <div style={{fontFamily: "monospace", fontSize: "12px", wordBreak: "break-all", color: "#2980b9"}}>{data?.hash || "0x..."}</div>
                </div>
            </div>
            ...
            <div style={{textAlign: "center", width: "150px"}}>
                <div style={{padding: "5px", background: "white", border: "1px solid #ddd"}}>
                    {/* Dùng thẻ img thay vì Component QRCode để tránh lỗi crash */}
...
// (Copy phần dưới này y hệt code cũ, chỉ thay đổi mỗi đoạn trên)
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // NHẬN DỮ LIỆU SỔ HỒNG TỪ SERVER (Có kiểm tra an toàn)
    socket.on("updateHouseStatus", (data) => {
      // Data có thể là string (cũ) hoặc object (mới) nên cần kiểm tra
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", position: "relative", fontFamily: "Segoe UI", background: "#111" }}>
      ...
      {/* ⚠️ KIỂM TRA: CHỈ RENDER NẾU CÓ DỮ LIỆU */}
        ...
        <div style={{ flex: 1, overflowY: "auto", fontSize: "13px" }}>
          {chatList.map((msg, i) => (
             <div key={i} style={{ marginBottom: "8px", textAlign: msg.id === socket.id ? "right" : "left" }}>
                ...
                <div style={{ background: msg.isPrivate ? "#fff3cd" : (msg.id === socket.id ? "#3498db" : (msg.role==='bot'?"#9b59b6":"#e1940fff")), color: msg.id === socket.id ? "white" : "black", padding: "8px 12px", borderRadius: "12px", display: "inline-block", maxWidth: "85%", border: msg.isPrivate ? "1px solid #e67e22" : "none", textAlign: "left" }}>
