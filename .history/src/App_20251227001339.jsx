...
// --- COMPONENT: SỔ HỒNG SỐ (GIỮ NGUYÊN CODE CŨ) ---
...
// --- [MỚI] COMPONENT: CHỌN KHÁCH HÀNG (Dành cho Broker) ---
function BuyerSelector({ players, onClose, onSelect }) {
    // Lọc danh sách: bỏ qua bản thân broker
    const clientList = Object.values(players).filter(p => p.id !== socket.id);

    return (
        <div style={{
            position: "fixed", top: "0", left: "0", width: "100%", height: "100%",
            background: "rgba(0,0,0,0.7)", zIndex: 9998, display: "flex", justifyContent: "center", alignItems: "center",
            fontFamily: "Segoe UI"
        }} onMouseDown={(e) => e.stopPropagation()}>
            <div style={{background: "white", padding: "20px", borderRadius: "10px", width: "320px"}}>
                ...
                <div style={{maxHeight: "200px", overflowY: "auto", border: "1px solid #eee", marginBottom: "15px"}}>
                    {clientList.length === 0 ? <div style={{padding:"10px", textAlign:"center", color:"#999"}}>Không có khách nào Online</div> : 
                    clientList.map(p => (
                        ...
                        }} onMouseEnter={e=>e.currentTarget.style.background="#f5f6fa"} onMouseLeave={e=>e.currentTarget.style.background="white"}>
                            <div style={{width:"10px", height:"10px", borderRadius:"50%", background:p.color}}></div>
                            <b>{p.name}</b>
                        </div>
                    ))}
                </div>
                <button onClick={onClose} style={{width:"100%", padding:"10px", border:"none", background:"#95a5a6", color:"white", borderRadius:"5px", cursor:"pointer"}}>Hủy bỏ</button>
            </div>
        </div>
    )
}

// --- GIỮ NGUYÊN: TÍNH TOÁN VAY VỐN ---
function LoanCalculator() {
  ...
  return (
    <div style={{ marginTop: "15px", paddingTop: "15px", borderTop: "1px dashed #ccc" }} onMouseDown={(e) => e.stopPropagation()}>
      <h4 style={{ margin: "0 0 10px 0", color: "#2980b9", display:"flex", alignItems:"center" }}>🏦 Bảng Tính Vay Ngân Hàng</h4>
      <div style={{display:"flex", justifyContent:"space-between", fontSize:"12px", marginBottom:"5px"}}><span>Giá trị nhà:</span><b>${price.toLocaleString()}</b></div>
      ...
      <div style={{ fontSize: "12px", marginBottom: "8px" }}>
        <div style={{display:"flex", justifyContent:"space-between"}}><label>Thời hạn vay:</label><span>{years} năm</span></div>
  ...
  return (
    <group position={[0, 1.6, 0]}>
      ...
      <mesh position={[2, 3, 2]} onClick={handleInfoClick}>
        ...
        <Billboard position={[0, 0.6, 0]}><Text fontSize={0.3} color="white" outlineWidth={0.05} outlineColor="#ff4757">INFO CHI TIẾT</Text></Billboard>
      </mesh>
      {showInfo && (
        <Html position={[2, 4, 2]} center zIndexRange={[100, 0]}>
          <div style={{ background: "white", padding: "20px", borderRadius: "12px", width: "350px", textAlign: "left", boxShadow: "0 20px 50px rgba(0,0,0,0.5)", fontFamily: "Segoe UI", pointerEvents:"auto", overflow:"hidden" }} onMouseDown={(e) => e.stopPropagation()}>
...
// --- [SỬA ĐỔI] BIỂN BÁO: Click gọi callback chứ không emit trực tiếp ---
function StatusSign({ position, role, status, onBrokerClick }) {
  const handleClick = (e) => {
    e.stopPropagation(); 
    if (role === "broker") {
        onBrokerClick(); // Gọi hàm xử lý của Broker
    } else {
        alert(status === "FOR SALE" ? "📢 Liên hệ SALES ADMIN để mua căn này!" : "🔒 Nhà đã bán. Miễn làm phiền.");
    }
  };
...
export default function App() {
  ...
  const [showBuyerSelector, setShowBuyerSelector] = useState(false); // State hiển thị Modal chọn khách
  ...
  useEffect(() => {
    ...
    // --- [SỬA ĐỔI] 1. Sự kiện Public: Chỉ nhận status chuỗi ("FOR SALE" hoặc "SOLD") ---
    socket.on("updateHouseStatus", (status) => {
      setHouseStatus(status);
      if (status === "FOR SALE") setCertificateData(null); // Reset nếu nhà bán lại
    });

    // --- [MỚI] 2. Sự kiện Private: Nhận Sổ hồng (Chỉ Broker & Buyer nhận đc) ---
    socket.on("showCertificate", (txData) => {
      setCertificateData(txData);
      document.exitPointerLock(); // Mở khóa chuột để xem sổ
      setIsLocked(false);
    });

    return () => { 
        socket.off("updatePlayers"); socket.off("receiveMessage"); socket.off("updateEnvironment"); 
        socket.off("updateLights"); socket.off("updateHouseStatus"); socket.off("showCertificate");
    };
  }, []);
  ...
  // [MỚI] Xử lý khi Broker click vào biển báo
  const handleBrokerClickSign = () => {
    if (houseStatus === "FOR SALE") {
        document.exitPointerLock();
        setIsLocked(false);
        setShowBuyerSelector(true); // Mở modal chọn khách
    } else {
        // Reset trạng thái nhà về FOR SALE
        if (window.confirm("Bạn muốn reset trạng thái nhà về ĐANG BÁN?")) {
            socket.emit("resetHouseStatus");
        }
    }
  };

  // [MỚI] Xử lý chốt đơn sau khi chọn khách
  const handleConfirmSale = (buyerId) => {
      setShowBuyerSelector(false);
      socket.emit("confirmTransaction", { buyerId }); // Gửi ID khách lên server
  };
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", position: "relative", fontFamily: "Segoe UI", background: "#111" }}>
      ...
      <Canvas camera={{ position: [0, 1.6, 10], fov: 60 }} shadows>
        ...
        {/* Truyền callback onBrokerClick vào StatusSign */}
